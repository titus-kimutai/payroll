$(document).ready(function() {
    ...
    $(document).on('change', 'input.product_unit_price', function() {
     ...
     $(document).on('change', 'input.product_inlinetax', function() {
        update_table_row($(this).closest('tr'));
    });
    $(document).on('change', 'input.product_discount', function() {
        update_table_row($(this).closest('tr'));
    });
...
function update_table_row(tr) {
    ...
    var pp = parseFloat(__read_number(tr.find('input.product_pp')));
    var disc = parseFloat(__read_number(tr.find('input.product_discount')));
    var taxrate = parseFloat(__read_number(tr.find('input.product_inlinetax')));
    var row_total = 0;
    var totaltx=0;
    var totaldisc=0;
    var amountplustax=pp;
    if (quantity && unit_price) {
         row_ttl = (quantity * pp);
        if (taxrate) {
            
        totaltx = (taxrate/100) * row_ttl;
         totaltxperunit = (taxrate/100) * pp;
        amountplustax=pp + totaltxperunit;
        
        } else{
          amountplustax=pp;  
        }
        if (disc) {
      totaldisc = (disc/100) * row_ttl;
        }
        row_total = ((quantity * pp)+totaltx)-totaldisc;
    }
    tr.find('input.product_line_total').val(__number_f(row_total));
    tr.find('input.product_unit_price').val(__number_f(amountplustax));
    update_table_total();
}
