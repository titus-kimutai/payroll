    ...
    public function getProfit($by = null)
    {
        ...
        $query->select(DB::raw('SUM(IF (TSPL.id IS NULL AND P.type="combo", ( 
            SELECT Sum((tspl2.quantity - tspl2.qty_returned) * (tsl.unit_price_inc_tax - pl2.purchase_price_inc_tax)) AS total
                ...
                (TSPL.quantity - TSPL.qty_returned) * (transaction_sell_lines.unit_price_inc_tax - PL.purchase_price_inc_tax)) )) AS gross_profit,PL.purchase_price_inc_tax as cost,transaction_sell_lines.unit_price_inc_tax as price,SUM(TSPL.quantity - TSPL.qty_returned) as quantity')
            );
