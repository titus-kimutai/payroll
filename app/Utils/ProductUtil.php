    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        $product = $query->select(
            ...
            DB::raw("(SELECT purchase_price_inc_tax FROM purchase_lines WHERE 
                        ...
                        variation_id=variations.id ORDER BY id DESC LIMIT 1) as last_purchased_price_minus_tax"),
                        DB::raw("(SELECT pp_without_discount FROM purchase_lines WHERE 
                        variation_id=variations.id ORDER BY id DESC LIMIT 1) as pp_without_discount"), 
                        DB::raw("(SELECT discount_percent FROM purchase_lines WHERE 
                        variation_id=variations.id ORDER BY id DESC LIMIT 1) as discount_percent"),
                        DB::raw("(SELECT item_tax FROM purchase_lines WHERE 
                        variation_id=variations.id ORDER BY id DESC LIMIT 1) as item_tax")
        )
        ->firstOrFail();
